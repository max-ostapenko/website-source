(() => {
    ...
    l = class {
      ...
      initContainer(e) {
        ...
        if (t.tagName === "INPUT") {
          ...
          t.innerHTML = "";
          let s = new r("form").class("pagefind-modular-input-wrapper").attrs({
            role: "search",
            "aria-label": "Search this site",
            action: "javascript:void(0);",
          });
          this.inputEl = new r("input").class("pagefind-modular-input").addTo(
            s,
          ),
        ...
        this.instance.on("filters", (t) => {
          ...
          this.available = Object.entries(s),
            Array.isArray(this.ordering)
              ? this.available.sort((n, d) => {
                let m = this.ordering.indexOf(n[0]),
                  _ = this.ordering.indexOf(d[0]);
                return (m === -1 ? 1 / 0 : m) - (_ === -1 ? 1 / 0 : _);
              })
              : this.available.sort((n, d) => n[0].localeCompare(d[0])),
  ...
  var F = async (i = 50) => await new Promise((e) => setTimeout(e, i)), c;
  try {
    c = new URL(document.currentScript.src).pathname.match(
      /^(.*\/)(?:pagefind-)?modular-ui.js.*$/,
    )[1];
  } catch {
    c = "/_pagefind/",
      console.warn(
        `Pagefind couldn't determine the base of the bundle from the javascript import path. Falling back to the default of ${c}.`,
      ),
      console.warn(
  ...
  var p = class {
    ...
    async __load__() {
      ...
      if (this.__initializing__ = !0, !this.__pagefind__) {
        let e = await import(`${this.options.bundlePath}pagefind.js`);
